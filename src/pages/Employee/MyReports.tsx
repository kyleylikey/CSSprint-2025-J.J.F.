...
export default function MyReports() {
  const { user } = useAuth();
  const { getReportsBySubmitter, reports } = useReports();
  
  // Get reports for current user (both identified and check all for demo)
  const userReports: Report[] = user 
    ? getReportsBySubmitter(user.id)
    : [];
  
  // For demo purposes, also show reports that match the user's name
  const allUserReports = user 
